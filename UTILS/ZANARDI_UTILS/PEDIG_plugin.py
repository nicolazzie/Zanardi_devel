...
def pedig_check(geno):
    ...
    matching=str(sum(geno[0][z]!=geno[1][z] for z in range(len(geno[0])) if geno[0][z]!='.' and geno[1][z]!='.'))
...
def run_pedigree_check (plink_pedfile, pedigree_file, mendel_thres, skipcouples, outfiles, checkall):
    ## Useful dicts 
    rehom={'AA':'A','BB':'B','CC':'C','GG':'G','TT':'T','11':'1','22':'2'}
    sesso={'M':'SIRE','F':'DAM'}
    ...
    passped.write('%19s; %19s; %4s; %12s; %9s\n' % ('ID_son','ID_parent','Type','MendErr/SNPs','MendErr%'))
    failped.write('%19s; %19s; %4s; %12s; %9s\n' % ('ID_son','ID_parent','Type','MendErr/SNPs','MendErr%'))
    suggest.write('%12s; %19s; %19s; %4s; %9s\n' % ('C.Num/T.Cand','ID_son','ID_candidate','Type','MendErr%'))
        ...
        for i in range(len(couples)):
            ...
            if pedcheck.has_key(couples[i][0]) and iid==couples[i][1]:
                ...
                if outcome <= mendel_thres:
                    cpass+=1
                    if couples[i][2]=='S':passped.write('%19s; %19s; SIRE; %12s; %9.5f\n' %  (couples[i][0],couples[i][1], '/'.join(mend_check[:2]),round(outcome,5)))
                    else:passped.write('%19s; %19s;  DAM; %12s; %9.5f\n' %  (couples[i][0],couples[i][1], '/'.join(mend_check[:2]),round(outcome,5)))
                else:
                    cfail+=1
                    if couples[i][2]=='S':
    ...
    if search_ped:
        for row in open(plink_pedfile):
            ...
            for i in search_ped:
                ...
                if fast_check[1] != '0' and int(fast_check[1]) > 100:  ## If no or low hom in fast comparison, do full comparison (few cases)
                    ...
                    if outcome > 10.:
                        if outcome < float(candid_ped[i]):candid_ped[i]=999;candid_id[i]=[('---',sesso[Sob[iid]])]
                        continue
                ...
                if outcome < float(candid_ped[i]):              #If better match, replace worse match
                    candid_ped[i]=outcome;candid_id[i]=[(iid,sesso[Sob[iid]])]
                elif outcome == float(candid_ped[i]):           #If same match, add new match to the list of candidates
        ...
        for ind in orderlist:
            ...
            for en,cands in enumerate(candid_id[ind]):
                candfound+=1
                num=str(en+1)
                mmatch=round(float(candid_ped[ind]),5)
                suggest.write('%12s; %19s; %19s; %4s; %9.5f\n' % (num+'/'+tot_cand,ind[0],candid_id[ind][en][0],candid_id[ind][en][1],mmatch))

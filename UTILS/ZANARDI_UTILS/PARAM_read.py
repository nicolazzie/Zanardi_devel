...
###############################
def inputfi(PARAMETERS,DEBUG,PEDIG,GSPREP):
    ...
    for variable in variables:
        ...
        if variable == variables[4]:
            if len(matching[1])==0 and (PEDIG or GSPREP):return(False,"Pedigree check requested, but no pedigree file provided. Please provide one in paramter file!")
            if len(matching[1])==0:continue
        ...
        if variable == variables[7]:
            if len(matching[1])==0 and GSPREP:return(False,"Pheno check requested, but no phenotype file provided. Please provide one in paramter file!")
            if len(matching[1])==0:continue
...
def plink_par (PARAMETERS,OUTDIR,output_name,DEBUG):
    ...
    plinkqc_vals[4] = OUTDIR+'/PLINK_OUT'+output_name
